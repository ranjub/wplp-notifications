...
<?php
    ...
    public function save_tickets_meta_box($post_id)
    {
        ...
        // Calculate affiliate cut for special statuses
        ...
        // Calculate and save affiliate percentage for the affiliate ID
        if (isset($_POST['affiliate_id'])) {
            $affiliate_id = intval($_POST['affiliate_id']);
            $task_amt_total = floatval(get_post_meta($post_id, 'task_amt_total', true));
            $affiliate_cut = floatval(get_post_meta($post_id, 'affiliate_cut', true));

            if ($task_amt_total > 0) {
                $affiliate_percentage = ($affiliate_cut / $task_amt_total) * 100;
            } else {
                $affiliate_percentage = 0;
            }

            update_user_meta($affiliate_id, 'affiliate_percentage', $affiliate_percentage);
        }
    ...
    public function show_affiliate_percentage_column($value, $column_name, $user_id)
    {
        if ('affiliate_percentage' == $column_name) {
            ...
            return $affiliate_percentage ? $affiliate_percentage . '%' : __('N/A', 'wplp_notification');
        }
        return $value;
    }
